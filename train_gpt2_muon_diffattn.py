...
3) Differential Attention official implementation:
...
# -----------------------------------------------------------------------------
# DiffAttention related:

# Core DiffAttention code 
def lambda_init_fn(depth):
    return 0.8 - 0.6 * math.exp(-0.3 * depth)

# No idea what this one does, the DiffAttn people had it
...
# -----------------------------------------------------------------------------
# RMSnorm (Also used in DiffAttention):
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.n_kv_heads = config.n_head // 2
        self.n_rep = 2
        
        self.head_dim = self.n_embd // config.n_head // 2
        self.scaling = self.head_dim ** -0.5
        
        self.q_proj = nn.Linear(self.n_embd, self.n_embd, bias=False)
        self.k_proj = nn.Linear(self.n_embd, self.n_embd // self.n_rep, bias=False)
        self.v_proj = nn.Linear(self.n_embd, self.n_embd // self.n_rep, bias=False)
    ...
    def forward(self, x):
        ...
        k = k.view(B, T, 2 * self.n_kv_heads, self.head_dim)
        v = v.view(B, T, self.n_kv_heads, 2, self.head_dim)
        ...
        if FLASH:
            ...
            k = k.reshape(B, T, self.n_kv_heads, 2, self.head_dim)
        
            q1, q2 = q[:, :, :, 0], q[:, :, :, 1]
            k1, k2 = k[:, :, :, 0], k[:, :, :, 1]
            v1, v2 = v[:, :, :, 0], v[:, :, :, 1]
            
            attn11 = F.scaled_dot_product_attention(q1, k1, v1, is_causal=True)
            attn12 = F.scaled_dot_product_attention(q1, k1, v2, is_causal=True)
            attn1 = torch.cat([attn11, attn12], dim=-1)

            attn21 = F.scaled_dot_product_attention(q2, k2, v1, is_causal=True)
            attn22 = F.scaled_dot_product_attention(q2, k2, v2, is_causal=True)
            ...
            attn = attn.reshape(B, T, self.n_head * 2 * self.head_dim)

            attn = self.c_proj(attn)
            return attn
        
        else:
            # manual implementation of DiffAttention
...
class GPT(nn.Module):
    ...
    def configure_optimizers(self, weight_decay, learning_rate, betas):
        ...
        for n, p in param_dict.items():
            if p.dim() == 2 and 'transformer.h.' in n:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_bin", type=str, default="dev/data/fineweb10B/fineweb_train_*.bin", help="input .bin to train on")
    parser.add_argument("--input_val_bin", type=str, default="dev/data/fineweb10B/fineweb_val_000000.bin", help="input .bin to eval validation loss on")
    ...
    for step in range(args.num_iterations + 1):
        ...
        print0(f"step {step+1:4d}/{args.num_iterations} | train loss {lossf:.6f} | norm {norm:.4f} | lr_scale {lr_scale:.2e} | ({(t1-t0)*1000:.2f} ms | {tokens_per_second:.0f} tok/s)")
        # log to logile
        if master_process and logfile is not None:
            with open(logfile, "a") as f:
                f.write("s:%d trl:%f\n" % (step, lossf))
